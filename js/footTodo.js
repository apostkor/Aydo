...
function HandleinputTodo(e) {
  ...
  if (data.data !== "") {
    ...
    localStorage.setItem(
      "currentFootData",
      JSON.stringify(currentFootTodolist)
    );
  }
    ...
    checkbox.addEventListener("click", function handleCheckbox() {
      if (checkbox.parentNode.className === "") {
        ...
        currentFootTodolist.forEach(function(data) {
          if (data.data === checkbox.nextSibling.data) data.value = true;
        });
      } else {
        checkbox.parentNode.className = "";
        currentFootTodolist.forEach(function(data) {
          if (data.data === checkbox.nextSibling.data) data.value = false;
        });
      }
...
function handelfootTodoButton(e) {
  ...
  if (currentFootData !== null && currentFootData.length !== 2) {
    ...
    closeFootTodoButton.addEventListener(
      "click",
      function HandlecloseFootTodoButton(e) {
        e.preventDefault();
        inputTodo.value = "";
        inputTodo.classList.add("hiding");
...
function handelsettings(e) {
  window.location.href = "https://github.com/apostkor";
}
